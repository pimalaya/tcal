...
The `merge` cargo feature gates one thing, the `pub mod merge` declaration. It pulls no crate into the build: ical-rs stopped being optional when calcard went, and it is now a plain dependency every configuration compiles.
...
The same removal landed in neverest this morning, for the same reason. This is tCal's half of it, done with tCard and tCal together so the two stay one shape.
