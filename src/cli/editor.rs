...
//! onto the calendar it was projected from.
//!
//! tCal writes the document to a temporary file, spawns the editor on its path
//! with every stream inherited, and reads the file back once the command
...
impl Editor<'_> {
    ...
    pub fn apply(
        ...
        let path = temp_dir().join(format!("tcal-{}.toml", Uuid::new_v4()));
        ...
        loop {
            let status = match spawn(&command, &path) {
                ...
                // what tCal put there a moment ago.
                Err(err) => {
                    remove(&path);
                    return Err(err);
                }
            ...
            let message = match &err {
                ...
                _ => return Err(keep(&path, err.into())),
            };
