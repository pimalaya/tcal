//! The format-preserving DOM: a [`Calendar`] of [`Nodes`], each a
//! [`Property`], a nested [`Component`], or a verbatim line.
...
impl Calendar {
    ...
    /// Every direct component of the given type, in document order.
    pub fn components(&self, ty: &str) -> impl Iterator<Item = &Component> {
        self.items.components(ty)
    }

    /// Every direct component of the given type, for in-place mutation.
    pub fn components_mut(&mut self, ty: &str) -> impl Iterator<Item = &mut Component> {
        self.items.components_mut(ty)
    }

    /// Ensure exactly `n` direct components of the given type at the root.
...
/// A node holding direct child components: the [`Calendar`] root or a
/// [`Component`]. Lets callers reconcile components the same way whether or
/// not the stream has a wrapping component.
pub trait Container {
...
impl Component {
    ...
    /// Ensure exactly `n` direct child components of type `ty` exist, new
    /// ones empty; fill them via [`Component::components_mut`].
    pub fn set_component_count(&mut self, ty: &str, n: usize) {
        let eol = eol_of(&self.begin_raw).to_owned();
        self.items.set_component_count(ty, n, &eol);
    }
...
/// One node of the tree: a property, a nested component, or an
...
/// An ordered list of [`Item`]s, shared by the calendar root and every
...
mod tests {
    ...
    fn applied(src: &str, name: &str, lines: &[&str]) -> String {
        ...
        let mut cal = Calendar::parse(src);
    ...
    fn round_trips_verbatim() {
        ...
        for src in [SAMPLE, folded, lf, bare, two, blanks] {
            assert_eq!(Calendar::parse(src).to_string(), src);
        }
    }
    ...
    fn set_component_count_appends_and_removes() {
        ...
        let mut cal = Calendar::parse(&grown);
        cal.component_mut("VEVENT")
            .unwrap()
